...
const makeDataFeed = () => {
  ...
  return {
    ...
    async getBars(
      ...
      // let to_test = Math.ceil(from.to);
      // let fromGetBars = Math.floor(from.from);
      // let toGetBars = Math.ceil(from.to);

      window.localStorage.setItem('resolution', resolution)
      resolution = convertResolutionToApi(resolution)

      // if (fromGetBars < minTs(symbolInfo.out_count, resolution)) {
      ...
      // if (fromGetBars < 1609459200) fromGetBars = 1609459200
      
      const key = `${symbolInfo.market}--${resolution}`

      // if (overTime[key] && overTime[key] > fromGetBars) {
      //   onHistoryCallback([], {nodeData: false})
      //   return
      // }
